...
void TreeNode::typeCheck(TreeNode* root) {
    ...
    case TreeNode::NODE_STMT:
        ...
        case StmtNode::STMT_FUNC:
            if(root->child)
            {   
                auto funSymbol = static_cast<FunStmtNode *>(root)->symbol;//C的类型转换机制,在强制类型转换的时候更加安全
        ...
        //输入的类型是否匹配
            string str = root->child->symbol->value;//int，char, 常量字符串的类型检查
    ...
    case TreeNode::NODE_OP:
        ...
        case OperatorNode::OP_AND://&&
            cl = static_cast<DeclNode *>(root->child)->symbol;//左孩子
            cr = static_cast<DeclNode *>(root->child->sibling)->symbol;//右孩子
            //and：两者类型不匹配，或者不是bool型
            if (cr->type != cl->type || cl->type != Symbol::VALUE_BOOL || cr->type != Symbol::VALUE_BOOL)
            {
                cerr << endl
                     << "type is error [" << Symbol::getSymbolName(cl)
                     << " , " << Symbol::getSymbolName(cr) << " ] or maybe they are not bool" << endl;
                assert(0);
            }
            ownSymbol = root->symbol = new OpNode(Symbol::VALUE_BOOL, "");
            break;
        ...
        case OperatorNode::OP_DSUB:
            ...
            break;
        case OperatorNode::OP_UMINUS://取负
            ownSymbol = root->symbol = new OpNode(Symbol::VALUE_INT, "");
            break;
        default:
            cl = static_cast<DeclNode *>(root->child)->symbol;
...
FunStmtNode::FunStmtNode(int lineno, Callable *symbol, std::initializer_list<TreeNode*> children) 
    : StmtNode (lineno, STMT_FUNC, children)
{
    this->symbol = symbol;
}
...
EmptyNode::EmptyNode(int lineno) 
    : StmtNode (lineno, STMT_EMPTY){ }

FuncCallNode::FuncCallNode(int lineno, Symbol *symbol, std::initializer_list<TreeNode *> children) 
    : ExprNode (lineno, symbol)
{
